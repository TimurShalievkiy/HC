...
public class FieldController : MonoBehaviour
{
    ...
    public bool CheckForInstance(List<int> posActivBlockInArr, Vector3 posOfFIrstBLock)
    {
        for (int i = 0; i < cellsStatePosArray.Length; i++)
        {
            if (Vector3.Distance(cellsStatePosArray[i].GetCerrentPosition(), posOfFIrstBLock) < 30)
            {
                Debug.Log(i);
                break;
